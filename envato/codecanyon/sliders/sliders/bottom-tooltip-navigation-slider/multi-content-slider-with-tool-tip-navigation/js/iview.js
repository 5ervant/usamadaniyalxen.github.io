 ...
 */ (function ($, window, undefined) {
	...
	var iView = function (el, options) {
			...
			//Set Timer Element
			iv.sliderContent.append('<div id="iview-timer"><div id="percentage"></div></div>');
			...
			$('.iview-video', iv.slider).click(function(e){
				...
				if(!$(e.target).hasClass('iview-video-close') && !$(e.target).hasClass('iview-caption') && !$(e.target).parents().hasClass('iview-caption')){
					video.show().animate({ top: 0 }, 1000, 'easeInCirc');
			...
			$('.iview-video-close', iv.slider).click(function(){
				...
				video.animate({ top: '-' + iv.defs.height + 'px' }, 1000, 'easeOutCirc', function(){
					video.hide();
	...
	iView.prototype = {
		...
		processTimer: function () {
			...
			iv.pieDegree += 3;
			var per = iv.pieDegree += 3;
			console.log(iv.pieDegree)
			
			$('#percentage').text('').text(Math.round((iv.pieDegree / 360)* 100)+'%');
		},
