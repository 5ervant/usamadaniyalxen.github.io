(function(LevelUtils){
	...
	LevelUtils.init=function(levelInstance){
		LevelUtils.vars.levelInstance=levelInstance;
		LevelUtils.vars.exitCell={
	...
	LevelUtils.getCell=function(x,y){
		var col=Math.floor(x/Config.CELL_WIDTH);
		var row=Math.floor(y/Config.CELL_HEIGHT);
		return {i:col,j:row};
	}
	...
	LevelUtils.getObject=function(cell){
		...
		for(var i=0;i<LevelUtils.vars.levelInstance.levelBoxes.length;i++){
			var box=LevelUtils.vars.levelInstance.levelBoxes[i];
	...
	LevelUtils.isExitCell=function(cell){
		if(cell.i==LevelUtils.vars.exitCell.i && cell.j==LevelUtils.vars.exitCell.j){
			return true;
		}
		return false;
	}
	...
	LevelUtils.callAction=function(currentCell, action,direction){
		LevelUtils.vars.levelInstance.characterAction(LevelUtils.getObject(currentCell),action,direction);
	}
