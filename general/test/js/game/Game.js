	...
	Game["preloaderState"]=function(){
		...
		hideAdBox=function(){
			if(document.getElementById("adBox")){
				document.getElementById("adBox").style.position="absolute";
				document.getElementById("adBox").style.display="none";
				document.getElementById("adBox").style.visibility="hidden";
				document.getElementById("adBox").style.top="-1000px";
				document.getElementById("adBox").style.zIndex=-10;
			}
		}
...
(function(Game){
	...
	function resize() { 
		...
		document.getElementById('adBox').style.left = (gameCanvas.width-336)/2+'px';
		document.getElementById('adBox').style.top = (375-280)*2/3+'px';
		
		if(gameCanvas.height<475){
			document.getElementById('adBox').style.MozTransform = 'scale(0.5)';
			document.getElementById('adBox').style.WebkitTransform  = 'scale(0.5)';
		}else{
			document.getElementById('adBox').style.MozTransform = 'scale(1)';
			document.getElementById('adBox').style.WebkitTransform  = 'scale(1)';
		}
		if(document.getElementById('adBox').style.display!="block" && Game.gameStarted!=true){
			document.getElementById('adBox').style.display="block";
			Game.gameStarted=true;
	...
	Game.stopSound=function(name){
		try{
			soundManager.getSoundById(name).stop();
		}catch(e){
			//
		}
	}
