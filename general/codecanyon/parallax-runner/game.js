/* ==============================================================
   ...
   Free Game: Parallax Runner - HTML5 Game
...
Canabalt.BIND_JUMP_KEYS = {
    '32': true
}; // spacebar

Canabalt.DISTANCE_TO_METERS_COEFFICIENT = 0.055;

Canabalt.PARALAX_BG_1_TOP_OFFSET = 150; // 100
Canabalt.PARALAX_BG_1_SPEED = 0.3;

Canabalt.PARALAX_BG_2_TOP_OFFSET = 100; // 100
...
Canabalt.prototype.start = function() {
    if (this.paused()) {
        ...
        this.interval = setInterval(function() {
            me.cycle();
        }, 1000 / Canabalt.CYCLES_PER_SECOND);
    }
    return this;
};
...
Canabalt.prototype.scheduleParalaxBeam = function(wait) {
    var me = this;
    this.paralaxBeamTimeout = setTimeout(function() {
        me.spawnParalaxBeam();
    }, wait);
};
...
Canabalt.prototype.draw = function() {
    ...
    // Draw paralax
    this.paralaxBg1.style.backgroundPosition = String(Math.round(this.paralaxBg1Offset)) + 'px ' + (Canabalt.PARALAX_BG_1_TOP_OFFSET - v) + "px";
    this.paralaxBg2.style.backgroundPosition = String(Math.round(this.paralaxBg2Offset)) + 'px ' + (Canabalt.PARALAX_BG_2_TOP_OFFSET - v) + "px";
...
Canabalt.Building = function(game, options) {
    ...
    this.width = bInit ? (window.innerWidth) ? window.innerWidth : document.body.clientWidth : 300 + Math.round(Math.random() * 1000);
    this.height = 30 + Math.round(Math.random() * 100);
    this.gap = Math.round(this.game.speed * 300);
    this.totalWidth = this.width + this.gap;

    this.left = bInit ? 1 : this.game.viewportWidth;
    if (bInit) this.game.y = this.height;
    bInit = 0;
...
Canabalt.Building.prototype.move = function(distance) {
    ...
    if (this.isIn) {
        ...
        // collision of man v building side here..
        //alert( this.game.y + " :: " + this.height)
        if ((0 + (this.game.y) + 16) < this.height) {
            if (Lives) Lives--;
            if (!Lives) gameEnd();
        }
    }
...
function fade(l) {
    document.getElementById("game").style.opacity = (l / 100);
    if (l) {
        l--;
        st = setTimeout("fade(" + l + ")", 24);
    } else {
        //
    }
}